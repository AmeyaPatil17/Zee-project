...
*/
 ...
 function renderForm($id, $q, $a, $b, $c, $d, $correct, $error)
 {
 ?>
 ...
 <input type="hidden" name="Q_no" value="<?php echo $id; ?>"/>
 <div>
...
$q = mysql_real_escape_string(htmlspecialchars($_POST['Question']));
 ...
 //error, display form
 renderForm($id, $q, $a, $b, $c, $d, $correct, $error);
 }
 else
 {
 // save the data to the database
 mysql_query("UPDATE civil SET Question='$q',  answer1='$a',  answer2='$b',  answer3='$c',  answer4='$d', correct='$correct' WHERE Q_no='$id'")
 or die(mysql_error()); 
 ...
 if (isset($_GET['Q_no']) && is_numeric($_GET['Q_no']) && $_GET['Q_no'] > 0)
 {
 // query db
 $id = $_GET['Q_no'];
 $result = mysql_query("SELECT * FROM civil WHERE Q_no=$id")
 or die(mysql_error()); 
 ...
 // show form
 renderForm($id, $q, $a, $b, $c, $d, $correct, '');
 }
 else

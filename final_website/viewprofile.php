...
<h3>NEWS</h3> 
  ...
  echo '<div>
    <p style="float: right;"><img src="images/beforeexam.png" height="580" width="250"></p>
    </div>';
  ...
  if (!isset($_GET['m_id'])) {
    $query = "SELECT email, f_name, m_name, l_name, gender, mobileno, b_date, branch FROM user WHERE m_id = '" . $_SESSION['user_id'] . "'";
  }
  else {
    $query = "SELECT email, f_name, m_name, l_name, gender, mobileno, b_date, branch FROM user WHERE m_id = '" . $_GET['m_id'] . "'";
  }
  ...
  echo '<p>&nbsp;</p>';
    ...
    if (!empty($row['gender'])) {
      echo '<tr><td class="label"><p>Gender:</p></td><td><p>';
      if ($row['gender'] == 'Male') {
        echo 'Male';
      }
    ...
    if (!empty($row['b_date'])) {
      if (!isset($_GET['m_id']) || ($_SESSION['user_id'] == $_GET['m_id'])) {
        // Show the user their own birthdate
        echo '<tr><td class="label"><p>Birthdate:</p></td><td><p>' . $row['b_date'] . '</p></td></tr>';
      }
      else {
        // Show only the birth year for everyone else
        list($year, $month, $day) = explode('-', $row['b_date']);
        echo '<tr><td class="label">Year born:</td><td>' . $year . '</td></tr>';
      }
	  echo'<tr><td><p></p></td></tr>';

    if (!empty($row['branch'])) {
      echo '<tr><td class="label"><p>Branch:</p></td><td><p>';
      if ($row['branch'] == 'CE') {
        echo 'CE';
      }
      else if ($row['branch'] == 'IT') {
        echo 'IT';
      }
	   else if ($row['branch'] == 'ETRX') {
        echo 'ETRX';
      }
	   else if ($row['branch'] == 'EXTC') {
        echo 'EXTC';
      }
    ...
    echo '</table>';
    if (!isset($_GET['m_id']) || ($_SESSION['user_id'] == $_GET['m_id'])) {
      echo '<p><p>Would you like to <a href="editprofile.php">edit your profile</a>?</p></p>';
    }
